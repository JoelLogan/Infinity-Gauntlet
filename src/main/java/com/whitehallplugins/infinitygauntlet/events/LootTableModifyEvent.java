...
public final class LootTableModifyEvent implements Modify {
    ...
    public void modifyLootTable(RegistryKey<LootTable> key, LootTable.Builder tableBuilder, LootTableSource source) {
        if (source.isBuiltin()) {
            if (WITHER_LOOT_TABLE_ID.equals(key.getValue())) {
            ...
            } else if (WARDEN_LOOT_TABLE_ID.equals(key.getValue())) {
                ...
                tableBuilder.pool(poolBuilder);
            } else if (ENDER_DRAGON_LOOT_TABLE_ID.equals(key.getValue())) {
                ...
                tableBuilder.pool(poolBuilder);
            } else if (ANCIENT_CITY_LOOT_TABLE_ID.equals(key.getValue())) {
                LootPool.Builder poolBuilder = getPoolBuilder(POWER_GEM.asItem(),
                        CONFIG.getOrDefault("powerGemRarityAncientCity", DefaultModConfig.POWER_GEM_RARITY_ANCIENT_CITY));
                tableBuilder.pool(poolBuilder);
            } else if (JUNGLE_TEMPLE_LOOT_TABLE_ID.equals(key.getValue())) {
                ...
                tableBuilder.pool(poolBuilder);
            } else if (DESERT_PYRAMID_LOOT_TABLE_ID.equals(key.getValue())) {
                ...
                tableBuilder.pool(poolBuilder);
            } else if (END_CITY_LOOT_TABLE_ID.equals(key.getValue())) {
                LootPool.Builder poolBuilder = getPoolBuilder(SPACE_GEM.asItem(),
                        CONFIG.getOrDefault("spaceGemRarityEndCity", DefaultModConfig.SPACE_GEM_RARITY_END_CITY));
                tableBuilder.pool(poolBuilder);
            } else if (WOODLAND_MANSION_LOOT_TABLE_ID.equals(key.getValue())) {
